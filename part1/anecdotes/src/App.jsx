import { useState } from 'react'

const Button = ({ onHandlerClick, text }) => (
  <button onClick={onHandlerClick}>
    {text}
  </button>
)

const Anecdote = (props) => {
  const {anecdotes, index, points} = props
  return(
    <>
    <h2>Anecdote of the day</h2>
    {anecdotes[index]}
    <br></br>
    <p>has {points[index]} votes</p>
    </>
  )
}

const AnecdoteMostVotes = (props) => {
  const {anecdotes, points} = props
  let mayor = points.indexOf(Math.max(...points))
  console.log(mayor)
  if(mayor === 0){
    return(
      <>
      <h2>Anecdote with most votes</h2>
      <p>There are no votes</p>
      </>
    )
  }
  return(
    <>
    <h2>Anecdote with most votes</h2>
    {anecdotes[mayor]}
    <br></br>
    <p>has {points[mayor]} votes</p>
    </>
  )
}
const App = () => {
  const anecdotes = [
    'If it hurts, do it more often.',
    'Adding manpower to a late software project makes it later!',
    'The first 90 percent of the code accounts for the first 10 percent of the development time...The remaining 10 percent of the code accounts for the other 90 percent of the development time.',
    'Any fool can write code that a computer can understand. Good programmers write code that humans can understand.',
    'Premature optimization is the root of all evil.',
    'Debugging is twice as hard as writing the code in the first place. Therefore, if you write the code as cleverly as possible, you are, by definition, not smart enough to debug it.',
    'Programming without an extremely heavy use of console.log is same as if a doctor would refuse to use x-rays or blood tests when diagnosing patients.',
    'The only way to go fast, is to go well.'
  ]
  let array = Array(anecdotes.length).fill(0)
  const [selected, setSelected] = useState(0)
  const [points, setPoints] = useState(array)
  const handleRandomClick = () => setSelected(Math.floor(Math.random() * anecdotes.length))
  console.log(points)
  const handlePointsClick = () => {
    const copy = [...points]
    copy[selected] += 1
    setPoints(copy)

  }
  return (
    <div>
      <Anecdote anecdotes={anecdotes} index={selected} points={points}/>
      <Button onHandlerClick={handleRandomClick} text="next anecdote" />
      <Button onHandlerClick={handlePointsClick} text={"vote"} />
      <AnecdoteMostVotes anecdotes={anecdotes} points={points}/>
      
    </div>
  )
}
export default App
